...
ADDENDUM_PATH = Path(__file__).parents[1] / 'configs' / 'agent_prompts' / 'icl_corpus_addendum.txt'
ADDENDUM_PLACEHOLDER = '{{ rag_corpus_dir }}'
...
def _parse_args():
    ...
    ap.add_argument('--rag-corpus-dir', required=True, help='Filesystem-RAG corpus dir (from build_rag_corpus.py)')
    ap.add_argument('--rag-tag', default=None, help='Suffix to disambiguate output dirs across ablations')
    ap.add_argument('--addendum-file', default=str(ADDENDUM_PATH), help='Prompt fragment appended to every task prompt')
...
def _resolve_paths(args):
    ...
    rag_tag = getattr(args, 'rag_tag', None) or ''
    variant_dir = f'iv_b_agent_icl-{variant}-{rag_tag}' if rag_tag else f'iv_b_agent_icl-{variant}'
    pred_dir = Path(args.pred_dir or f'results/predictions/{variant_dir}/{target_short}')
    metrics_dir = Path(args.metrics_dir or f'results/metrics/{variant_dir}/{target_short}')
...
def _load_rag_addendum(addendum_file: str, rag_dir: Path) -> str:
    text = Path(addendum_file).read_text(encoding='utf-8')
    return text.replace(ADDENDUM_PLACEHOLDER, str(rag_dir))
...
def run_shard(args, shard_id: int, num_shards: int):
    ...
    rag_tag = getattr(args, 'rag_tag', None) or ''
    variant_label = f'iv_b_agent_icl-{variant}-{rag_tag}' if rag_tag else f'iv_b_agent_icl-{variant}'
    print(f'{tag}{variant_label}/{target_short}  model={args.model}  test_field={args.test_field}')
    print(f'{tag}  Repo:        {args.repo_path}')
    print(f"{tag}  RAG corpus:  {args.rag_corpus_dir}  (tag={rag_tag or '∅'})")
    ...
    rag_dir = Path(args.rag_corpus_dir).resolve()
    if not (rag_dir / 'INDEX.txt').is_file():
        raise SystemExit(f'[iv_b_agent_icl] RAG corpus not initialized at {rag_dir} (no INDEX.txt). Run: python precompute/build_rag_corpus.py')
    rag_addendum = _load_rag_addendum(args.addendum_file, rag_dir)
        ...
        with open(pred_path, 'a') as f:
            for i, (record, commit_id, parent_id) in enumerate(instances, 1):
                ...
                if 'InternalServerError' in err_str or 'APIConnectionError' in err_str or 'Connection error' in err_str:
                    ...
                    consec_server_err = 0
                entry = {'cve_id': record['cve_id'], 'commit_id': commit_id, 'parent_id': parent_id, 'method': 'agent_icl_filesystem', 'variant': variant_label, 'test_field': args.test_field, 'model': args.model, 'rag_corpus_dir': str(rag_dir), 'rag_tag': rag_tag or None, 'predicted_functions': result['predictions'], 'raw_predictions': result['raw_predictions'], 'agent_steps': result['agent_steps'], 'agent_cost': result['agent_cost'], 'elapsed_s': result['elapsed_s'], 'error': result['error']}
...
def aggregate_and_save(args):
    ...
    print_overall(result['overall'], f'{variant_label} ({args.test_field}, {args.model})')
...
def main():
    ...
    if getattr(args, 'metrics_only', False):
        ...
        rag_tag = getattr(args, 'rag_tag', None) or ''
        variant_label = f'iv_b_agent_icl-{variant}-{rag_tag}' if rag_tag else f'iv_b_agent_icl-{variant}'
        print(f'[metrics-only] {variant_label}/{target_short}  target={args.target}  pred_dir={pred_dir}')
